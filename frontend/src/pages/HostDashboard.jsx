...
export default function HostDashboard() {
  ...
  async function handleCreate(e) {
    ...
    if (!topic) {
      setFormError("Enter a custom topic");
      return;
    }
    if (!name) {
    ...
    const { question_time, intermission_time, host_controlled } =
      loadGameSettings();

    setGenerating(true);
    try {
      const token = await getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/create-quiz`, {
        method: "POST",
        headers: {
        ...
        <div className="mb-8 animate-fade-in">
          ...
          <p className="text-gray-500 text-sm">
            ...
            <span className="font-semibold text-gray-300">
              {user?.firstName || user?.username || "Host"}
            </span>
            <span className="text-gray-600 ml-2">
              · Game settings via ⚙️ in nav
            </span>
          </p>
        </div>
          ...
          {historyLoading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-16 bg-gray-800 rounded-2xl animate-pulse"
                />
              ))}
            </div>
          ) : history.length === 0 ? (
