...
function SessionCard({ session, index }) {
  ...
  const avgScore =
    totalPlayers > 0 && final_leaderboard?.length > 0
      ? Math.round(
          final_leaderboard.reduce((s, e) => s + e.score, 0) /
            final_leaderboard.length,
        )
      : null;
  const isCloseGame =
    topN.length >= 2 && maxScore > 0
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap mt-0.5">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-bold capitalize ${diffColors.bg} ${diffColors.text}`}
                >
                  {difficulty}
                </span>
                  ...
                  <div key={entry.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-sm font-black text-gray-300 ml-2 flex-shrink-0">
...
export default function History() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/my-history`, {
  ...
  }, [fetchHistory, isSignedIn, isLoaded]);
    ...
    return (
      <main className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex flex-col items-center justify-center py-24 gap-6 animate-fade-in text-center">
          ...
          <div>
            <h1 className="text-2xl font-black text-white mb-2">Game History</h1>
            <p className="text-gray-400 text-sm">Sign in to view the games you've hosted and their results.</p>
          </div>
          <SignInButton mode="modal">
            <button className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold rounded-2xl transition-all shadow-lg">
    ...
    <main className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-gray-300">
          {totalGames > 0
            ? `${totalGames} session${totalGames !== 1 ? "s" : ""}`
            : "Sessions"}
        </h2>
        <button
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-32 bg-gray-800/60 rounded-3xl animate-pulse"
            />
          ))}
        </div>
      ) : error ? (
        ...
        <div className="text-center py-16 animate-fade-in">
          ...
          <p className="text-gray-600 text-sm">
            Create your first quiz from the dashboard!
          </p>
        </div>
      ) : (
        <div className="space-y-4">
